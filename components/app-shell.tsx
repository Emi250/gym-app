    ...
    <div className="flex min-h-svh flex-col">
      ...
      >
        <div className="mx-auto flex h-14 max-w-md items-center gap-1 px-2">
          ...
          <h1 className="flex-1 truncate px-2 text-lg font-semibold tracking-tight">
            {title ?? "Gym Tracker"}
          </h1>
          {topRight ?? <SyncBadge />}
        </div>
...
function BackButton({ back }: { back: AppShellProps["back"] }) {
  ...
  const cls =
    "text-fg-muted hover:bg-bg-elevated flex h-10 w-10 items-center justify-center rounded-full";
  if (back === true) {
    return (
